...
def predict_next_day(symbol, seq_length=60):
    ...
    checkpoint = torch.load(f"models/lstm_{symbol}.pth", map_location=torch.device('cpu'))
    model = LSTMModel()
    ...
    change_percent = (change / current_price * 100) if current_price not in (None, 0) else None
